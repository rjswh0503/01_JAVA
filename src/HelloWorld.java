...
/*
  ...
  public을 쓰는 이유 : main은 프로그램에서 기본이기 떄문에 public을 사용한다.
 ...
 메소드란(method) : 어떤 특정 작업을 수행하기 위한 명령문의 집합
  ...
  main에서 static을 사용하는 이유는 프로그램의 기본이 되어야하는 함수가 
  정리되면 안되기 때문에 static 영역에 메모리로 할당을 시켜주는 것. 
  
      
  void : 반환이 없다는 의미 (return이 없음) 메인이 죽으면 다음 단계가 없이 
  프로그램 전체가 종료되므로, return을 가질 수 없음
